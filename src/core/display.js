 ...
 *
 * @return {Object<Display>}        instance of Display that control Game
 *                                  instance view.
 */
export class Display {
   ...
   * @param  {Number} scale           positive number to scale the game
   *                                  for better view
   */
    ...
    for (let i = 0; i < this.level.height; i++) {
      for (let k = 0; k < this.level.width; k++) {
        ...
        if (wall) {
          ...
          ctx.fillRect(k * this.scale,
                       i * this.scale + this.topSpace,
   ...
   * @param  {Number} score   Positive number that describe the score
   * @param  {Number} lives   Positive number that describe the lives 1 to 3
   * @param  {Number} level   Positive number that describe the level number
   *
   * @return { Boolean }
   */
  drawFrame(score, lives, level) {
    ...
    this.level.actors.forEach((actor) => {
      ...
      if (actor.type === 'food') {
        ctx.beginPath();
        ctx.arc(posX + actorWidth / 2,
                posY + actorHeight / 2,
                actorWidth / 2, 0, Math.PI * 2);
      ...
      if (actor.type === 'snake') {
        actor.parts.forEach((part, i) => {
          ...
          let partY = part.y * this.scale + this.topSpace +
                      actor.size.y * this.scale * .05;
  ...
  drawInfo(ctx, score, lives, level) {
    ...
    ctx.fillText('simple HTML & JS game',
                 this.wrap.width / 2, this.scale * 4.5);

    ctx.font = this.scale + 'px pixelated, serif';
    ctx.textAlign = 'right';
    ctx.fillStyle = this.bgColor;
    ctx.fillText('HI-SCORE: ' + hiScore,
                 this.wrap.width - this.scale, topSpace + this.scale * 8 / 10);

    ctx.textAlign = 'center';
    ctx.fillText('SCORE: ' + score,
                 colWidth * 2, topSpace + this.scale * 8 / 10);
  ...
  message(message) {
    ...
    let height = (lines.length + 2) * this.scale;
    let width = 15 * this.scale;
    let x = this.wrap.width / 2;
    let y = this.wrap.height /2;
